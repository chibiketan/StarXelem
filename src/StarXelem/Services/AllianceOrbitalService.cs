...
public class AllianceOrbitalService : IAllianceOrbitalService
{
    ...
    /// <summary>
    /// Synchronise les items pour un profil RSI par batches de 5000 max avec un syncedAt commun.
    /// Gère les codes HTTP : 200 (succès), 401/403 (token/profil invalide), 400 (validation).
    /// </summary>
    public async Task<List<SyncResult>> SyncItemsAsync(string rsiProfilGuid, List<ItemSyncItem> items, DateTime syncedAt)
    {
        var token = await _settingsService.GetAsync("ApiKey");
        ...
        // Découpe en batches de 5000 items max, tous avec le même syncedAt (cohérence API)
        var results = new List<SyncResult>();
        var batches = items.Chunk(5000);

        foreach (var batch in batches)
        {
            // Body : { rsiProfilGuid, syncedAt: ISO 8601, items: batch }
            var requestBody = new { 
                rsiProfilGuid, 
            ...
            try
            {
                using var content = new StringContent(
                    System.Text.Json.JsonSerializer.Serialize(requestBody), null, "application/json");
            ...
            if (response.StatusCode == System.Net.HttpStatusCode.OK)
            {
                ...
                var result = System.Text.Json.JsonSerializer.Deserialize<SyncResult>(json, options);
                _logger.LogInformation("Synchronisation d'objets batch réussie : {Received} reçus, {Matched} reconnus, {Updated} mis à jour, {Removed} supprimés, {Filtered} rejetés",
                    result?.Received ?? 0, result?.Matched ?? 0, result?.Updated ?? 0, result?.Removed ?? 0, result?.Filtered ?? 0);
                results.Add(result ?? new SyncResult());
            }
