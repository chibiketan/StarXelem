...
public static class ObjectGraphRetrieval
{
    public static void Run(GrpcChannel channel, Metadata authHeaders)
    {
        var entityGraphService = new EntityGraphService.EntityGraphServiceClient(channel);

        var request = new InventoryQueryRequest();
        
        var response = entityGraphService.InventoryQuery(request, authHeaders);
