...
- `Services/Mining/IMineralSignatureExtractor` — extracts per-mineral base radar signatures from `MineableRock_*` p4k entities; `MineralSignatureGenerator` expands each into a cluster-size (1-10) signature table (`Data.MineralSignatureEntity`, populated in `LocalDatabaseService` rebuild phase 11)
- `Services/Scan/*` — screen-scan pipeline for the signature-recognition overlay: `IScreenCaptureService` (GDI capture of the game window), `ISignatureOcrService` (Windows OCR + SkiaSharp preprocessing), `IScanTriggerService` implemented by `IGlobalHotkeyService` (Win32 `RegisterHotKey` on a dedicated thread) and `IJoystickTriggerService` (DirectInput polling via Vortice.DirectInput; only one is active per `ScanTriggerSettings.Kind`), `IOverlayNotificationService` (borderless, non-activating result window), orchestrated by `IScanSignatureOrchestrator`. See `Documents/Plans/2026-09-07-scan-signature-overlay.md` and `Documents/Datafiles/MineralSignatures.md`.
